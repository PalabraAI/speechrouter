...
- **Endpoint override.** `SPEECHROUTER_PALABRA_WS_BASE` (full stream URL, no query) wins over `SPEECHROUTER_PALABRA_REGION`; empty by default, so production stays on the region table. The upstream host is logged on connect and in connect failures (never the token).
